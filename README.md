...
所有消息都经过精心挑选，传递温柔、被看见与被接纳的关怀力量。

### 🌿 疗愈模式开关

- 该模式默认开启：将少量“偏励志、用力”的句子柔化为更疗愈的表达。
- 如需关闭，可在 `暖心便签弹窗阵列.py` 顶部将 `HEALING_MODE = True` 改为 `False`。
...
### 为什么仓库里没有 dist 文件夹/可执行文件？

- dist/ 与 build/ 属于“可再生成”的构建产物，已在 `.gitignore` 中忽略，避免仓库体积膨胀。
- 如需下载可执行文件，请前往仓库的 [Releases](https://github.com/Iwillbecomestrong/nuanxinbiaoqian/releases) 页面获取对应版本（例如 v1.1.0）。
- 如果你想本地自行构建，可参考上文“🛠️ 自己打包”章节。
